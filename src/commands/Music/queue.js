...
module.exports = {
   ...
   if(!player) return message.channel.send({ embeds: [new MessageEmbed().setColor(client.embedColor).setTimestamp().setDescription(`Nothing is playing right now.`)]});
            ...
            if(player.queue.length === "0" || !player.queue.length) {
                ...
                const queuedSongs = player.queue.map((t, i) => `\`[${++i}]\` \`[${convertTime(t.duration)}]\` - ${t.title} - [${t.requester}]`);
                ...
                if(player.queue.size < 11) {
                    const embed = new MessageEmbed()
                    .setColor(client.embedColor)
                    .setAuthor({ name: `${message.guild.name} queue`, iconURL: message.guild.iconURL() })
                    .setDescription(`Now Playing: **[${player.queue.current.title}](${player.queue.current.uri})**`)
                    .addField(`Queued Songs`, `${pages[page]}`)
                    .setTimestamp()
                    ...
                    .setEmoji("<:emoji_38:981078094146334770>")
                    .setStyle("SECONDARY")

                    const but2 = new MessageButton()
                    .setCustomId("queue_cmd_but_2")
                    .setEmoji("<:emoji_37:981078068758216736>")
                    .setStyle("SECONDARY")
