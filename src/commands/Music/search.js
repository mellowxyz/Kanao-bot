    ...
    switch (s.loadType) {
        case "TRACK_LOADED":
            ...
            const embed = new MessageEmbed()
             ...
             .setDescription(`\`[${convertTime(s.tracks[0].duration, true)}]\` \`-\` [${s.tracks[0].title}](${s.tracks[0].uri}) \`${convertTime(s.tracks[0].duration, true)}\``)
             .setColor(client.embedColor)
             .setTimestamp()
             .setFooter({ text:`Requested by ${client.author.tag}` })
         ...
         case "SEARCH_RESULT":
             ...
             const results = s.tracks.slice(0, 5).map(x => `\`[${index++}]\` \`[${convertTime(x.duration)}]\` - [${x.title}](${x.uri})`)
                    .join("\n");
            ...
            collector.on("collect", async (b) => {
                ...
                if(b.customId === "s_one") {
                    player.queue.add(s.tracks[0]);
                        ...
                        if(msg) await msg.edit({embeds: [search.setTitle(`Added to queue`).setDescription(`\`[${convertTime(s.tracks[0].duration, true)}]\` - [${s.tracks[0].title}](${s.tracks[0].uri}) `).setFooter(`Requested by ${message.author.tag}`).setTimestamp()]})
                } else if(b.customId === "s_two") {
                    player.queue.add(s.tracks[1]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setTitle(`Added to queue`).setDescription(`\`[${convertTime(s.tracks[1].duration, true)}]\` - [${s.tracks[1].title}](${s.tracks[1].uri}) `).setFooter(`Requested by ${message.author.tag}`).setTimestamp()]})
            
                } else if(b.customId === "s_three") {
                    player.queue.add(s.tracks[2]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setTitle(`Added to queue`).setDescription(`\`[${convertTime(s.tracks[2].duration, true)}]\` - [${s.tracks[2].title}](${s.tracks[2].uri}) `).setFooter(`Requested by ${message.author.tag}`).setTimestamp()]})
            
                } else if(b.customId === "s_four") {
                    player.queue.add(s.tracks[3]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setTitle(`Added to queue`).setDescription(`\`[${convertTime(s.tracks[3].duration, true)}]\` - [${s.tracks[3].title}](${s.tracks[3].uri}) `).setFooter(`Requested by ${message.author.tag}`).setTimestamp()]})
            
                } else if(b.customId === "s_five") {
                    player.queue.add(s.tracks[4]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setTitle(`Added to queue`).setDescription(`\`[${convertTime(s.tracks[4].duration, true)}]\` - [${s.tracks[5].title}](${s.tracks[4].uri}) `).setFooter(`Requested by ${message.author.tag}`).setTimestamp()]})
